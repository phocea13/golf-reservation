...
export class ReservationService {
    ...
    async clickTimeOfDay(timeOfDay: string = 'morning') {
        ...
        if (morningButton === null) {
            console.log('Morning button is not available, clicking on the next date then come back later');
        } else {
            ...
            console.log('Morning button text:', morningText);
    ...
    async clickTimeSlot(): Promise<boolean> {
        ...
        for (const timeSlot of timeSlots) {
            ...
            if (timeSlotText?.trim().startsWith("10:") || timeSlotText?.trim().startsWith("11:")) {
